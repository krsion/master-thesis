...
**Metadata reduction via causal stability.** Each event carries a vector clock with $O(P)$ entries for $P$ peers. The `CausalStabilityTracker` (implemented and wired into the sync server) identifies causally stable events. Bauwens and Gonzalez Boix [@bauwens2020stability] show that stable entries can have their clock metadata pruned on the *client* side, reducing per-event overhead. Server-side PO-Log pruning (discarding stable events) is not feasible because the relay server cannot materialize documents and replay references prevent unconditional pruning. Client-side clock pruning avoids both issues: it reduces metadata size without requiring materialization or removing events from the DAG.
