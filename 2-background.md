...
- **Frontiers** are used as event parents (encoding causal dependencies), for synchronization (a peer sends its frontier, the other peer walks the DAG to compute the missing events), and for materialization (Kahn's topological sort algorithm needs the actual graph edges to determine replay order --- vector clocks alone do not encode which events are direct parents vs transitive ancestors).
