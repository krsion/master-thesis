...
- **Frontiers** (stored as explicit parent pointers on each event) are used as event parents, for synchronization (a peer sends its frontier, the other peer walks the DAG to compute the missing events), and for materialization (Kahn's topological sort traverses the parent edges directly). In principle, the DAG structure could be reconstructed from vector clocks alone by computing the transitive reduction of the happens-before relation, but storing parent pointers explicitly makes DAG traversal straightforward without this computation.
