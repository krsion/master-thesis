...
Eg-walker [@gentle2025egwalker] is a collaborative text editing algorithm that overcomes the OT/CRDT trade-off. OT is simple (plain integer indices, no per-character metadata) but merging long-diverged branches is O(n²). CRDTs handle diverged branches efficiently via unique character identifiers, but incur per-character overhead even in the steady state.

Eg-walker stores operations in a *causal event graph* and materializes the document by replaying them in a deterministic topological order. Sequential edits use plain indices (like OT); only concurrent forks trigger CRDT-style merging, after which the temporary state is discarded.
