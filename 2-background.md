...
**Loro** [@loro] solved the wrap problem with a native movable-tree CRDT [@kleppmann2021move] where moves are atomic. However, Loro uses opaque node IDs rather than path-based addressing. This caused the **retargeting problem**: when replaying a recorded edit sequence on a different subtree, relative references (e.g., `../../0/contact/source`) cannot be expressed --- Loro's ID points to the specific node from the original recording, not to the structurally equivalent node in the new context. After a `wrapRecord` operation moves the target one level deeper, the ID still points to the wrapper, not to the `source` child. This is a fundamental mismatch between ID-based and path-based addressing.
