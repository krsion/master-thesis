...
To isolate the $P$ factor, we measured 2 active peers with varying numbers of inactive peers (all registered in the DAG but not editing concurrently). With $C_\text{total}$ held constant, the overhead grows linearly in $P$ up to $P \approx 200$, confirming the $O(NP + C_\text{total})$ prediction. Beyond $P = 500$, cache and memory effects cause superlinear degradation. This confirms that the constant-$P$ assumption is appropriate for Denicek's target use case of small-group collaboration.
