...
To isolate the $P$ factor, we measured 2 active peers with varying numbers of inactive peers (all registered in the DAG but not editing concurrently). With $C_\text{total}$ held constant, the overhead grows linearly in $P$ up to $P \approx 200$, but becomes superlinear for larger $P$: at $P = 1000$ the workload is 31$\times$ slower than $P = 2$ despite identical $C_\text{total}$. This is because vector clock operations (merge during event creation, dominance checks during validation) are $O(P)$ each and are called $O(P)$ times per sync, contributing an $O(P^2)$ term that becomes dominant at large peer counts. For Denicek's target use case of small-group collaboration ($P \leq 20$), this overhead is negligible.
