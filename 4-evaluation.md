...
**Materialization cost is quadratic for concurrent branches.** Linear extensions (the common case during local editing) extend a cached document in place at O(1) amortized cost. Geometric checkpoints allow rematerialization to resume from the longest matching prefix of the topological order, but the `resolveAgainst` step still scans all prior events for each event being replayed, giving $O(N^2)$ in the worst case for a workload dominated by concurrent branching.
