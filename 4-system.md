...
**Reliability** is achieved through frontier-based catch-up: each sync message includes frontiers, so dropped connections are recovered by resending missing events on reconnection. Duplicate events are detected by event ID and ignored. Out-of-order events are buffered until their parents arrive. **Compaction** materializes the document and discards old events when all active peers have acknowledged the same frontier. The PO-Log is otherwise append-only because replay references event IDs that must remain in the DAG ([@Sec:pure-op-crdt]).
