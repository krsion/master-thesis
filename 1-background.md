...
The framework assumes *reliable causal broadcast* [@baquero2017pureop, §3]: delivery is causal (if $a \to b$, then $a$ is delivered before $b$), and every operation is eventually delivered. It also defines a mechanism for pruning *causally stable* operations --- those that have been delivered to all replicas --- from the PO-Log, since they can no longer affect the outcome of future merges.
