...
The framework assumes *reliable causal broadcast* [@baquero2017pureop, §3]: delivery is causal (if $a \to b$, then $a$ is delivered before $b$), and every operation is eventually delivered. It also defines a mechanism for pruning *causally stable* operations (delivered to all replicas) from the PO-Log.

mydenicek implements this framework directly. The mapping is:

| Baquero | mydenicek |
|---------|-----------|
| PO-Log | Event DAG stored as per-peer arrays of events, each event having a list of parent references |
