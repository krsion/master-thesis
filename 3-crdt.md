...
**Proof sketch.** `materialize` is pure: it composes `topologicalOrder` (deterministic — `EventId` comparison is a strict total order, no iteration-order dependence), `resolveAgainst` (pure — sequential walk dispatching stateless class methods), and `apply` (pure — local mutations, no randomness). Same inputs, same output. $\square$

**Strong eventual consistency** follows: the G-Set ensures replicas eventually hold the same event set; deterministic *eval* produces the same document. TP1/TP2 are not needed — the replay order is fixed by the DAG.
