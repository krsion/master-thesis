...
**Limited list operations.** mydenicek provides only `pushFront`, `pushBack`, `popFront`, and `popBack` for lists --- there are no general `insert(index, item)` or `remove(index)` operations. This limits expressiveness: users cannot insert at an arbitrary position or remove a specific item by index. The push/pop pairs also interact poorly with concurrency: as discussed in [@Sec:crdt-framing], a concurrent `pushFront` and `popFront` can cancel each other out depending on replay order, because `popFront` always targets index 0 at replay time rather than tracking which item it intended to remove. General indexed insert and remove would allow richer list manipulation and could provide better concurrent semantics through index transformation. Additionally, the original Denicek supports a `reorder` operation for rearranging list items, which mydenicek does not implement. Reorder would require a conflict-resolution strategy for concurrent reorders of the same list (e.g., a movable-list CRDT) and is left as future work.
