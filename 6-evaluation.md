...
**Strict-index and negative-index concurrency semantics.** The `insert` and `remove` operations accept an optional `strict` flag for positive indices. When `strict=true`, the index is fixed and not shifted by concurrent OT --- this matches the `!N` convention used in selectors. For example, `insert(target, 0, value, true)` always inserts at the front. Negative indices (e.g. `-1`, `-2`) use Python-style end-relative addressing resolved at replay time, so `insert(target, -1, value)` always appends and `remove(target, -1)` always removes the last item, regardless of concurrent insertions or removals. A concurrent strict-front-insert and strict-front-remove can still cancel each other out --- this is inherent to the "always index 0" semantic, not a bug. The non-strict positive (default) variants provide better concurrent behavior because their indices are transformed through concurrent operations. The `reorder(target, from, to)` operation implements drag-and-drop with concurrent index transformation, resolving conflicting reorders deterministically.
