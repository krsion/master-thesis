...
**Strict-index concurrency semantics.** The `insert` and `remove` operations accept an optional `strict` flag. When `strict=true`, the index is fixed and not shifted by concurrent OT --- this matches the `!N` convention used in selectors. For example, `insert(target, 0, value, true)` always inserts at the front, and `insert(target, -1, value, true)` always appends, regardless of concurrent edits. Strict negative indices are resolved at replay time against the actual list length; strict positive indices are used as-is. A concurrent strict-front-insert and strict-front-remove can still cancel each other out --- this is inherent to the "always index 0" semantic, not a bug. Non-strict indices (the default) --- both positive and negative --- are shifted through concurrent operations and provide better intent preservation. Each list edit event carries a `listLength` field recording the target list's length at creation time, enabling OT transforms to resolve non-strict negative indices to absolute positions and shift them through concurrent edits just like positive indices. The `reorder(target, from, to)` operation implements drag-and-drop with concurrent index transformation, resolving conflicting reorders deterministically.
...
**Formula arguments use the same reference mechanism as the rest of the tree.** Formula nodes contain `ReferenceNode` children whose selectors are rewritten by `updateReferences` after structural edits (rename, wrap, reorder) --- the same mechanism that retargets all references in the tree. Because the formula engine operates directly on the internal `Node` tree, references are resolved transparently via `ReferenceNode.resolveReference` and `Node.navigate`, with no separate path-parsing step. The formula engine never distinguishes references from inline values --- an `evaluateChild` helper resolves references before any evaluator sees the argument, so retargeting is entirely a concern of the edit pipeline, not the evaluation pipeline. An earlier design used a separate `$ref` string convention for formula arguments, which required its own retargeting logic; this was eliminated by making formulas use `ReferenceNode` like everything else.
