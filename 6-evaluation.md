...
**Formula references are retargeted through structural edits.** When a formula node contains a `$ref` argument (e.g., `$ref: "../input"`) and a structural edit renames or wraps the referenced field, the `$ref` path is rewritten to follow the structural change. Operation-based formulas (`$tag: "x-formula"`) are handled by `rewriteFormulaArgReferences`, which runs after each structural edit's `apply()` method and resolves `$ref` paths relative to the formula node's position. Tag-based formulas (e.g., `split-first`, `x-formula-plus`) whose `$ref` children are direct record fields are handled by the existing `updateReferences()` mechanism. In both cases, the formula engine itself never distinguishes references from inline values --- an `evaluateChild` helper resolves `$ref` transparently before any evaluator sees the argument, so the retargeting is entirely a concern of the edit pipeline, not the evaluation pipeline. The combined effect is that formula references survive concurrent structural edits in all cases tested by the formative examples and the property-based test suite.
