...
**Push/pop concurrency semantics.** The end-relative operations `pushFront`/`popFront` and `pushBack`/`popBack` can interact unexpectedly under concurrency: a concurrent `pushFront` and `popFront` may cancel each other out depending on replay order, because `popFront` always targets index 0 at replay time (see [@Sec:crdt-framing]). The general `insertAt` and `removeAt` operations provide better concurrent semantics through explicit index transformation, but the push/pop operations remain for convenience and backward compatibility. Additionally, the original Denicek supports a `reorder` operation for drag-and-drop rearrangement; mydenicek's `ListReorderEdit` implements this with concurrent index transformation, resolving conflicting reorders deterministically.
...
**Formula references in operation-based formulas are retargeted through structural edits.** When a formula node (`$tag: "x-formula"`) contains a `$ref` argument (e.g., `$ref: "../input"`) and a structural edit renames or wraps the referenced field, the `$ref` path is rewritten to follow the structural change. This is implemented by `rewriteFormulaArgReferences`, which runs after each structural edit's `apply()` method and resolves `$ref` paths relative to the formula node's position --- matching the formula engine's own resolution semantics. Tag-based formulas (e.g., `split-first`, `x-formula-plus`) whose `$ref` children are direct record fields are handled by the existing `updateReferences()` mechanism. The combined effect is that formula references survive concurrent structural edits in all cases tested by the formative examples and the property-based test suite.
