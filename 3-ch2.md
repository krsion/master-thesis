# More complicated chapter {#chap:math}

You can use \LaTeX{} as you like in markdown! If it's easier to do it in \LaTeX{} then do it.

After the reader gained sufficient knowledge to understand your problem in [@chap:refs], you can jump to your own advanced material and conclusions.

You will need definitions (see [@defn:x] below in [@sec:demo]), theorems ([@thm:y]), general mathematics, algorithms ([@alg:w]), and tables ([@tbl:z] or [@Tbl:z])\todo{See documentation of package \texttt{booktabs} for hints on typesetting tables. As a main rule, \emph{never} draw a vertical line.}. [@Fig:f;@Fig:g] show how to make a nice figure. See [@fig:schema] for an example of TikZ-based diagram. Cross-referencing helps to keep the necessary parts of the narrative close --- use references to the previous chapter with theory wherever it seems that the reader could have forgotten the required context. Conversely, it is useful to add a few references to theoretical chapters that point to the sections which use the developed theory, giving the reader easy access to motivating application examples.

## Example with some mathematics {#sec:demo}
...
: An example table. Table caption should clearly explain how to interpret the data in the table. Use some visual guide, such as boldface or color coding, to highlight the most important results (e.g., comparison winners). {#tbl:z}

![A figure with a plot, not entirely related to anything. If you copy the figures from anywhere, always refer to the original author, ideally by citation (if possible). In particular, this picture --- and many others, also a lot of surrounding code --- was taken from the example bachelor thesis of MFF, originally created by Martin Mareš and others.](img/ukazka-obr02.pdf){width=60% #fig:g}
...
Most importantly, do \underline{not} overuse bold text, which is designed to literally **shine from the page** to be the first thing that catches the eye of the reader. More precisely, use bold text only for 'navigation' elements that need to be seen and located first, such as headings, list item leads, and figure numbers.

Use underline only in dire necessity, such as in the previous paragraph where it was crucially important in ensuring that the reader remembers to never typeset boldface text manually again.
