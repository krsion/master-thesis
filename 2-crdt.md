...
- **Checkpoint-based resumption.** When another peer's incoming event invalidates the linear cache (because its parents do not match the current frontier), the materializer saves the current cached document together with its topological order as a *checkpoint*. On the next rematerialization, `findBestCheckpoint` looks for a checkpoint whose saved topological order is a *prefix* of the new topological order. If one exists, replay resumes from that point --- skipping all events already covered by the prefix --- rather than replaying from the initial document. This is a best-effort optimization: if the concurrent event's peer ID sorts before the local peer's, lexicographic tie-breaking inserts it in the middle of the existing order, breaking the prefix property and forcing a full replay. The checkpoint helps most when the local peer has the lower ID, or when causal dependencies already constrain the order so that new events land at the end.
