...
**Full replay (no cache).** Topological sort runs Kahn's algorithm with a binary-heap tie-breaker in $O(N \log N)$ time (edges are $O(N P)$ but $P$ is bounded). For the $i$-th event, the per-peer index finds concurrent priors in $O(P \log N)$ (binary search per peer) plus $O(C_i \log C_i)$ for sorting by topological position. The overall `resolveAgainst` cost is:
$$\sum_{i=1}^{N} O(P \log N + C_i \log C_i)$$
Let $C_\text{total} = \sum C_i$ be the total number of concurrent pairs. The cost simplifies to $O(N P \log N + C_\text{total} \log N)$. In a fully sequential graph ($C_\text{total} = 0$), the cost is $O(N P \log N)$. In a fully concurrent merge-fan ($C_\text{total} = O(N^2)$), the cost is $O(N^2 \log N)$. Each `apply` navigates a selector path and mutates the document; the cost depends on the path length and affected region, bounded by $O(D)$ in the worst case. Space is $O(N P)$ for the event map plus $O(D)$ for the materialized document, plus $O(N)$ for the per-peer index.
