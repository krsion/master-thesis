...
- **Replay.** Each event's edit is applied in order, after transforming it through all incomparable predecessors. Each pairwise transformation is $O(1)$ (fixed dispatch on edit type). Since events are stored per peer and sequence numbers are contiguous, finding the concurrent boundary is $O(1)$ per peer, $O(P) = O(1)$ total. Concurrent events from all peers are merged in topological order by scanning $P$ advancing pointers; with $P$ constant, this adds $O(1)$ per transformation. Cost: $O(N + C_\text{total})$, where $C_\text{total}$ is the number of incomparable pairs in the poset --- the total number of pairwise transformations actually performed. Without the constant-$P$ assumption, the full cost is $O(NP + C_\text{total})$: the $NP$ term comes from scanning $P$ peer boundaries per event.

The overall cost is $O(N + C_\text{total})$ with $P$ constant, or $O(NP + C_\text{total})$ in general.
