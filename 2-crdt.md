...
The `resolveAgainst` step is the heart of convergence. When the materializer is about to apply event E, it iterates over all previously applied events. For each prior event P, it first checks whether E's vector clock dominates P's --- if so, P is a causal ancestor of E and is skipped. Otherwise, if neither clock dominates the other, the events are concurrent, and the materializer calls `transformLaterConcurrentEdit(P.edit, E.edit)`, which rewrites E's selector (and potentially its payload) through P's structural effect. Transformations compose: if E is concurrent with priors P₁, P₂, and P₃, the edit is first transformed through P₁, then the result through P₂, then through P₃. After all priors have been processed, the materializer checks whether the final transformed edit can still be applied to the current document state (via `canApply`). If not --- for example, because a concurrent delete removed the target node --- the edit is downgraded to a no-op conflict.
...
**Sync with geometric checkpoints.** A peer with a local history of $N$ events receives $B$ concurrent events from another peer. The linear cache is invalidated and `materialize` must replay. The topological sort costs $O((N + B) \log(N + B))$. The geometric checkpoint cache typically covers the first $K$ events of the new topological order (the shared causal prefix --- events that both peers had seen before they diverged). Replay resumes from position $K$, so only $R = N + B - K$ events are replayed. For the $j$-th replayed event ($j = 1, \ldots, R$), `resolveAgainst` scans $K + j - 1$ prior entries, yielding:
$$\sum_{j=1}^{R} O((K + j) \cdot P) = O(R \cdot K \cdot P + R^2 \cdot P)$$
Checkpointing avoids redoing the $\Theta(K^2 P)$ `resolveAgainst` work for the shared prefix. In the typical sync scenario, the shared prefix $K$ is close to $N$ (everything before the fork), so $R \approx B$ and the cost drops from $O((N+B)^2 P)$ to $O(B \cdot N \cdot P + B^2 \cdot P)$. During replay, $O(\log R)$ geometric checkpoints are saved, each requiring a document clone at $O(D)$, adding $O(D \log R)$ overhead. Additional checkpoint-cache space is $O(D \log N)$ for the retained document snapshots.

**Summary.** The amortized cost per `insertEvent` + `materialize` call is $O(D)$ for local editing. After sync with $B$ concurrent events, a single $O(B \cdot N \cdot P)$ rematerialization restores the cache, and subsequent local edits return to $O(D)$ amortized cost. The worst case remains $O(N^2 P)$ when the entire graph consists of concurrent events (no shared causal prefix), but this is unlikely in practice: Denicek sessions involve few peers and short offline intervals.
