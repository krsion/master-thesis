...
The sync server is deployed on Azure Container Apps. We chose Container Apps over both serverless platforms (Azure Functions, AWS Lambda) and traditional PaaS (Azure App Service): serverless platforms impose per-request timeouts that break long-lived WebSocket sessions, while App Service runs the container continuously and bills accordingly even when no peers are connected. Container Apps runs long-lived processes without request timeouts and scales to zero when idle, so a deployed but unused room incurs no compute cost. The NDJSON event log is persisted to a mounted Azure Files share, since the container's local filesystem is ephemeral and would be lost on each restart or scale-to-zero event. The main drawback is cold-start latency: the first peer to connect after a period of inactivity waits a few seconds while the container is pulled and started.
