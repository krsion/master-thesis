...
- **Checkpoint-based merge resumption.** When a merge invalidates the linear cache (because the incoming event's parents do not match the current frontier), the materializer saves the current cached document as a *checkpoint* keyed by the pre-merge frontier. It then rematerializes from scratch --- but the next time `materialize` is called with the same event set, it can resume from that checkpoint instead of replaying from the initial document. In effect, only the events added after the fork point need to be replayed and resolved. This is particularly useful after sync: the first materialization after receiving a batch of concurrent events pays the full cost, but subsequent calls (e.g., as the user continues editing locally) restart from the checkpoint rather than from the beginning of history.
