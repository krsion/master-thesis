...
The system was validated on five formative examples, 331 tests (including a concurrent pair matrix, property-based convergence testing with `fast-check`, and intention preservation invariants). All examples are available as interactive demos at <https://krsion.github.io/mydenicek>.
...
**Client-side metadata pruning.** Tracking causal stability --- identifying events that all peers have observed --- would allow pruning vector clock entries on clients, reducing per-event metadata from $O(P)$ to $O(1)$ for stable events [@bauwens2020stability].
