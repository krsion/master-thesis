...
**Scaling to large peer counts.** The complexity analysis treats $P$ as a constant (2--5 peers). For larger groups, vector clock operations become $O(P)$ and start to matter. Analyzing and optimizing materialization for large $P$ and large documents is an open problem.
