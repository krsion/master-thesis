...
**Proof (paper proof).** Let $e_1, e_2, \ldots, e_n$ be the topological order produced by Lemma 1, and let $d_0$ be the initial document. Define $d_k$ and $a_k$ (the applied-prefix list after step $k$) inductively:
...
Note that this is a *paper proof* --- it establishes the logical structure of the convergence argument but delegates implementation-level determinism to the audit in [@Sec:determinism-audit]. Mechanical verification (e.g., in TLA+ or a proof assistant) is left as future work (see [@Sec:future-work]).
...
One edit type is deliberately excluded from undo: `CopyEdit`. A faithful inverse would have to restore every subtree that was overwritten at the (possibly wildcard-expanded) target, which requires reading the entire `preDoc` subtree and emitting a composite replace-edit. This is implementable but adds significant complexity to the undo path, and the cost falls on every `preDoc` snapshot whether or not it is ever used. Instead, `CopyEdit.computeInverse` throws a descriptive error; callers that know they may produce copy events must either avoid calling `undo()` after a copy or wrap the undo in their own try/catch.
...
A key concern is preserving edits that a peer made offline. The `resetToCompactedState` method handles this by saving the peer's unsynced local events before replacing the event graph with the compacted state. After ingesting any remaining events from the server's response, the saved edits are re-applied against the new compacted state. If a saved edit no longer applies --- for instance, because its target was deleted during the compacted history --- the re-apply fails silently and the edit is dropped. This mechanism ensures that offline edits survive compaction in the common case while maintaining convergence in the edge case where the offline edits conflict with the compacted history.
