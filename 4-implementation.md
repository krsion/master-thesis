...
**Lemma 2 (pure selector resolution).** *`resolveAgainst(ev, applied, doc)` is a pure function of its three inputs.* The method only reads the input event's clock, walks the `applied` list sequentially, and calls each prior edit's `transformLaterConcurrentEdit` method, which is dispatched on the edit's class and has no hidden state.
...
**Proof.** By Lemma 1, both replicas produce the same topological order. By Lemmas 2 and 3, each step of the replay is a pure function of (event, applied-prefix, document). A straightforward induction on the length of the ordered sequence shows that the materialized document after step $k$ is a function only of $\mathcal{E}$. Taking $k$ to be the length of the sequence gives the theorem. $\square$

The Lemma-level determinism assumptions are *implementation obligations*: no dependence on hash-map iteration order, no hidden mutable state, no floating-point non-determinism. They are audited in [@Sec:determinism-audit].
