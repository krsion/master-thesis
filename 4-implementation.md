...
The deployment builds a Docker image via Azure Container Registry, then deploys it using a Bicep infrastructure-as-code template. Event data is persisted to an Azure Files *share* --- a network-mounted file system, analogous to an SMB network drive --- attached to the container. Azure Files was chosen over Blob Storage or Table Storage because it provides a POSIX file system interface, allowing the sync server to append events to NDJSON files directly without needing a storage SDK. A managed database (Azure SQL, Cosmos DB) would provide indexed queries and transactional guarantees, but carries a fixed monthly cost regardless of usage --- disproportionate for a thesis prototype where the entire persistence requirement is appending immutable events to a log. Azure Files storage is significantly cheaper and matches the append-only nature of the event DAG.
