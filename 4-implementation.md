...
**Why a single-threaded event loop, not Workers or a thread pool.** The sync server uses Deno's single-threaded event loop to process all rooms in a single process. This is sufficient for the thesis prototype because: (a) the typical workload is 2--5 peers per room with fewer than 100 events per session, where each sync message processes in under a millisecond; (b) rooms are independent --- no shared state between them --- so a single event loop can interleave messages from different rooms without locks; (c) adding Deno Workers (separate JavaScript threads with isolated heaps) would require serializing room state across thread boundaries on every message, adding latency and complexity for no benefit at this scale. For a production deployment with hundreds of concurrent rooms, a Worker-per-room or process-per-room architecture (similar to Cloudflare Durable Objects) would prevent one room's heavy merge from blocking others. The current architecture is the simplest correct design for the targeted use case.
...
**Disk persistence.** Room state is written to disk as a JSON file after every sync message. Writes are asynchronous and chained per room: while one write is in progress, subsequent sync messages queue their writes, and only the latest state is persisted --- intermediate states are skipped. This means the server never blocks on I/O during sync processing, but a crash between sync and persistence completion could lose the most recent events. On restart, rooms are reloaded from the last persisted state.
