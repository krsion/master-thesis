...
**Lemma 2 (pure selector resolution).** *`resolveAgainst(ev, applied, doc)` is a pure function of its three inputs.* The method only reads the input event's clock, walks the `applied` list sequentially, and calls each prior edit's `transformLaterConcurrentEdit` method, which is dispatched on the edit's class and has no hidden state. Concretely, the fifteen edit types fall into three groups: (a) data edits (`RecordAddEdit`, `RecordDeleteEdit`, `ListPushBackEdit`, `ListPushFrontEdit`, `ListPopBackEdit`, `ListPopFrontEdit`, `SetEdit`, `ApplyPrimitiveEdit`) return the selector unchanged; (b) structural edits (`RecordRenameFieldEdit`, `UpdateTagEdit`, `WrapRecordEdit`, `WrapListEdit`, `ListInsertAtEdit`, `ListRemoveAtEdit`, `ListReorderEdit`) rewrite the selector and optionally the payload via `transformLaterConcurrentEdit`; and (c) `CopyEdit` rewrites both its target and source selectors. Each group's transformation logic reads only its own fields and the input selector --- no mutable globals, no iteration-order-dependent collections.
...
**Concurrent structural conflicts.** Several concurrent scenarios deserve explicit discussion because they exercise the conflict-resolution semantics most directly. *Concurrent renames of the same field* to different names (Alice renames `name` to `fullName`, Bob renames `name` to `title`): the first rename in replay order succeeds; the second rename's selector is transformed through the first --- its source `name` becomes `fullName` --- so it renames `fullName` to `title`. The last rename in replay order wins: the field ends up under `title`. *Concurrent wraps of the same target* (Alice wraps `value` into a `formula` record, Bob wraps `value` into a `container` record): the first wrap succeeds and adds a path segment; the second wrap's selector is transformed through the first wrap, gaining an extra segment, and wraps the already-wrapped node a second level deep. This produces a doubly-wrapped structure --- the original value is nested inside two wrapper records instead of one. Neither peer intended this nesting; each expected a single wrapper. This is a known semantic compromise: the system preserves both peers' structural edits rather than discarding one, at the cost of producing a deeper tree than either peer intended. In practice, concurrent wraps of the exact same node are uncommon; when they do occur, the extra nesting is visible in the UI and can be corrected manually by unwrapping one level. *Concurrent insert and remove at the same index* (`insertAt("items", 0, "NEW")` vs. `removeAt("items", 0)`): the insert shifts the remove's target up, so the remove deletes the original item at index 0 and the insert places `"NEW"` at index 0 --- both edits take effect independently, yielding `["NEW", "b", "c"]`. This is the correct behavior regardless of replay order because `removeAt` carries an explicit index that is transformed through the concurrent insert. By contrast, the end-relative operations `pushFront`/`popFront` interact less predictably: a concurrent `pushFront` and `popFront` can cancel each other out depending on replay order, because `popFront` always targets index 0 at replay time rather than tracking which item it intended to remove. The indexed operations `insertAt`/`removeAt` are therefore preferred for concurrent scenarios. These behaviors follow mechanically from the selector-rewriting rules and the deterministic replay order; they are convergent by construction, though whether they always match user intent is application-dependent.
...
| `ListInsertAtEdit` | Insert an item at a given index | List |
| `ListRemoveAtEdit` | Remove the item at a given index | List |
...
- **InsertAt**: shifts numeric indices at or above the insertion point (e.g., `insertAt(items, 2, ...)` transforms `items/3` to `items/4` but leaves `items/1` unchanged).
- **RemoveAt**: shifts numeric indices above the removal point down by one, and marks the exact index as removed (e.g., `removeAt(items, 2)` transforms `items/3` to `items/2`, and a concurrent edit targeting `items/2` becomes a no-op conflict).
...
A naive transformation implementation requires a rule for every pair of edit types --- $n^2$ rules for $n$ edit types. With 15 edit types, that would be 225 hand-written rules. mydenicek avoids this through a two-level object-oriented design, shown in [@Fig:edit-class-diagram].
...
A subtlety arises when undo interacts with concurrent remote events. The inverse edit is computed against the document state at the *original event's parent frontier* --- that is, the state just before the undone edit was first applied --- not the current document state. This is causally correct: the undo cancels exactly the effect of the original edit, regardless of what other events have been applied concurrently. However, the resulting behavior may be unintuitive in some scenarios. For example, if Alice adds a field and Bob concurrently modifies a sibling field, Alice's undo removes her addition but does not interact with Bob's modification --- even if the two fields are semantically related. The undo event is then a new event in the DAG, and other peers apply it through the normal materialization pipeline. This design ensures convergence (the undo is just another event in the G-Set) but means that undo under concurrency is a causal inverse, not a state rollback.

`CopyEdit` requires special handling because it overwrites a (possibly wildcard-expanded) target subtree. Its `computeInverse` snapshots every target node before the copy, producing a `RestoreSnapshotEdit` (or a `CompositeEdit` of multiple `RestoreSnapshotEdit` instances for wildcard targets) that restores each overwritten subtree on undo. This snapshot-based inverse incurs a per-copy overhead proportional to the size of the overwritten subtrees, but the cost is acceptable because copy operations typically target small nodes (e.g., copying a string from an input field). For wildcard copies, each expanded target is individually snapshotted and restored.
...
Initial documents are validated by hash: the first client to sync with a room sets the room's initial document hash, and subsequent clients must match it. Room creation is serialized by the Deno runtime's single-threaded event loop: the synchronous code within the bootstrap handler runs atomically between `await` suspension points, so two peers connecting simultaneously cannot both set a different initial document. Even in the degenerate case where both arrive at the bootstrap check concurrently, the hash comparison rejects any mismatch after the fact.
