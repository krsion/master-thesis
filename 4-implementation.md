...
The document is not the state; it is a **pure view function** $\text{materialize}: 2^{\text{Event}} \to \text{Node}$ computed on demand. The view function is the composition of three pure steps described above: topological ordering with an `EventId` tie-break, `resolveAgainst` (selector rewriting through concurrent prior edits), and `apply` (executing the resolved edit against the current document). In Baquero's terminology, the event DAG is the *PO-Log* (partially ordered log of operations), `materialize` is the *eval* function, and our server-side compaction ([@Sec:compaction-offline]) corresponds to *PO-Log pruning* via a redundancy relation over causally stable operations. The main structural difference is that our PO-Log stores explicit parent pointers forming a DAG, which enables checkpoint-based incremental materialization --- Baquero's framework only requires causal metadata for the partial order.
...
The classical TP1 and TP2 properties [@ressel1996integrating] are not needed: they are conditions for peer-local OT where peers apply operations in different local orders. In mydenicek, the replay order is fixed by the DAG, so commutativity is not required. The fixed order can produce different "winners" than a TP1-satisfying system (the peer with the lexicographically smaller ID wins ties), but for Denicek's primary use case --- structural transformations via wildcards --- this is the right trade-off: the wildcard-affects-concurrent-insertions semantics ([@Sec:wildcard-concurrent]) actually *improves* intention preservation by automatically applying schema evolution to concurrently inserted items.
