...
Container Apps was chosen as the best fit: minimal operational overhead, zero cost at rest, and sufficient for a single-container research deployment. For comparison, the cheapest App Service plan with unlimited WebSocket connections (Basic B1) costs approximately \$100/month regardless of usage, while Container Apps under the consumption plan incurs no cost when idle and includes a generous free monthly grant. The trade-off is a *cold start* delay: when the container has scaled to zero, the first WebSocket connection takes a few seconds while the container starts up.
