...
*Concurrent strict insert and remove.* With strict indices (`strict=true`) or negative indices, the index is *not* shifted by OT --- it always refers to the position at replay time, not at creation time. A concurrent `insert("items", 0, "NEW", true)` and `remove("items", 0, true)` can cancel each other out: if the insert replays first, the remove targets index 0, which is now the *newly inserted* item rather than the original. The result is `["a", "b", "c"]` --- back to the original state. The same cancellation occurs with negative end-relative operations (e.g. `insert(-1, ...)` and `remove(-1)`). This is inherent to the "always the same position" semantics of strict/negative indices. For concurrent list modifications where both peers intend to affect specific existing items, the non-strict (OT-shifted) positive-index variants provide better intent preservation.
...
- **Insert**: shifts numeric indices at or above the insertion point (e.g., `insert(items, 2, ...)` transforms `items/3` to `items/4` but leaves `items/1` unchanged). Strict-index inserts (`strict=true`) and negative-index inserts do not shift concurrent selectors --- negative indices are resolved to absolute positions at replay time, and the resolved version then shifts later edits normally.
- **Remove**: shifts numeric indices above the removal point down by one, and marks the exact index as removed (e.g., `remove(items, 2)` transforms `items/3` to `items/2`, and a concurrent edit targeting `items/2` becomes a no-op conflict). Strict-index and negative-index removes do not shift concurrent selectors until resolved.
