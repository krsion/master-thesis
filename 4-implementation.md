...
**Proof sketch.** Same input to a deterministic function yields the same output. The topological order is a total function of the event set (Kahn's algorithm on the DAG, with ties broken by the lexicographic order of `EventId`s, which are unique by construction). `resolveAgainst` and `apply` are pure functions of their inputs. Hence $\text{materialize}$ is a deterministic function of $\mathcal{E}$, and equality of event sets implies equality of materialized documents. $\square$

In particular, mydenicek's edits do **not** need to be commutative, and the classical OT transformation properties TP1/TP2 [@ressel1996integrating] do not apply. TP1 and TP2 are correctness conditions for *peer-local* application of operations as they are received, where two peers may apply $a$ and $b$ in opposite orders and must still reach the same state. In mydenicek every replica derives the document from the full event set via the same canonical replay, so commutativity is not required for convergence. What the proof *does* require is that `topologicalOrder`, `resolveAgainst`, and `apply` are genuinely deterministic functions at the implementation level --- no dependence on hash-map iteration order, no hidden mutable state, no floating-point non-determinism. This implementation-level property is audited in [@Sec:determinism-audit].
